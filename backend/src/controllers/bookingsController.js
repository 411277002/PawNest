...
const DEFAULT_TIMES = ['10:00', '10:30', '11:00', '13:30', '14:00', '15:30', '16:00', '18:30'];
...
function roleCanModify(req, booking) {
  if (req.user.role === 'customer') {
    return CUSTOMER_MODIFIABLE_STATUSES.includes(booking.status) && canCustomerModify(booking.booking_date, booking.start_time);
  }
  if (STAFF_ROLES.includes(req.user.role)) return !hasServiceStarted(booking.booking_date, booking.start_time);
  return false;
}
...
function ensureBookingOwnerOrStaff(req, booking) {
  ...
  if (req.user.role === 'customer') return Number(booking.customer_id) === Number(req.user.id);
...
function getBoardingFullMessage(petType) {
  if (petType === 'cat') return '此日期貓咪住宿房已滿';
  if (petType === 'dog') return '此日期狗狗住宿房已滿';
  return '此日期住宿房已滿';
}
...
async function loadActiveServices(serviceIds, connection = pool) {
  ...
  const [services] = await connection.execute(
    `
    SELECT id, name, category, price, duration_minutes, description, target_pet_type
    FROM services
...
async function loadPetForUser(petId, userId, connection = pool) {
  const [rows] = await connection.execute('SELECT id, type FROM pets WHERE id = ? AND user_id = ? LIMIT 1', [petId, userId]);
  return rows[0];
}

async function loadPetById(petId, connection = pool) {
  const [rows] = await connection.execute('SELECT id, type FROM pets WHERE id = ? LIMIT 1', [petId]);
  return rows[0];
}
...
async function loadStoreCapacity(storeId, connection = pool) {
  const [rows] = await connection.execute(
    `
    SELECT id, dog_room_capacity, cat_room_capacity, daycare_capacity
    FROM stores
    WHERE id = ?
    LIMIT 1
    `,
...
async function insertBookingServices(connection, bookingId, services) {
  for (const service of services) {
    await connection.execute(
      `
      INSERT INTO booking_services (booking_id, service_id, price_snapshot, duration_snapshot)
      VALUES (?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE
        price_snapshot = VALUES(price_snapshot),
        duration_snapshot = VALUES(duration_snapshot)
      `,
      [bookingId, service.id, Number(service.price || 0), Number(service.duration_minutes || 0)],
    );
  }
}

async function getBoardingUsedCount({ storeId, bookingDate, petType, excludeBookingId = null, connection = pool }) {
  const params = [storeId, normalizeDate(bookingDate), petType];
  let extra = '';
  if (excludeBookingId) {
    extra = 'AND b.id <> ?';
...
async function getDaycareUsedCount({ storeId, bookingDate, excludeBookingId = null, connection = pool }) {
  const params = [storeId, normalizeDate(bookingDate)];
  let extra = '';
  if (excludeBookingId) {
    extra = 'AND b.id <> ?';
    params.push(excludeBookingId);
...
async function checkCapacity({ storeId, bookingDate, petType, services, excludeBookingId = null, connection = pool }) {
  ...
  if (needsBoardingCapacity(services)) {
    ...
    if (capacity <= 0) return { unavailable: true, reason: getBoardingNoCapacityMessage(petType) };
    const used = await getBoardingUsedCount({ storeId, bookingDate, petType, excludeBookingId, connection });
    if (used >= capacity) return { unavailable: true, reason: getBoardingFullMessage(petType) };
  }

  if (needsDaycareCapacity(services)) {
    const capacity = Number(store.daycare_capacity || 0);
    if (capacity <= 0) return { unavailable: true, reason: '此門市尚未設定安親名額' };
    const used = await getDaycareUsedCount({ storeId, bookingDate, excludeBookingId, connection });
    if (used >= capacity) return { unavailable: true, reason: '此日期安親名額已滿' };
  }

  return { unavailable: false };
}

async function isSlotUnavailable({ store_id, booking_date, start_time, excludeBookingId = null }) {
  const time = normalizeTime(start_time);
  const date = normalizeDate(booking_date);
  const [blocks] = await pool.execute(
    `SELECT id FROM booking_time_blocks WHERE store_id = ? AND block_date = ? AND start_time = ? LIMIT 1`,
    [store_id, date, time],
  );
  if (blocks[0]) return { unavailable: true, reason: '此時段已關閉' };
  ...
  const [rows] = await pool.execute(
    ...
    FROM bookings
    WHERE store_id = ? AND booking_date = ? AND start_time = ?
      AND status NOT IN ('cancelled', 'completed')
      ${extra}
    `,
    params,
  );
  if (Number(rows[0]?.total || 0) >= SLOT_CAPACITY) return { unavailable: true, reason: '此時段已額滿' };
  return { unavailable: false };
}

export async function getBookingOptions(req, res) {
  const [pets] = await pool.execute(
    `SELECT id, name, type, breed, gender, age, weight, note, photo_url FROM pets WHERE user_id = ? ORDER BY created_at DESC, id DESC`,
    [req.user.id],
  );
  const [services] = await pool.execute(
    `SELECT id, name, category, price, duration_minutes, description, target_pet_type FROM services WHERE status = 'active' ORDER BY FIELD(category, 'grooming', 'boarding', 'daycare', 'addon'), id ASC`,
  );
  const [stores] = await pool.execute(
    `SELECT id, name, area, address, phone, open_time, close_time, dog_room_capacity, cat_room_capacity, daycare_capacity FROM stores WHERE status = 'active' ORDER BY id ASC`,
  );
...
export async function getAvailability(req, res) {
  ...
  if (!storeId || !bookingDate) return res.status(400).json({ message: '請選擇門市與日期' });

  const [blocks] = await pool.execute('SELECT start_time, reason FROM booking_time_blocks WHERE store_id = ? AND block_date = ?', [storeId, bookingDate]);
  const blockMap = new Map(blocks.map((row) => [String(row.start_time).slice(0, 5), row.reason || '此時段已關閉']));

  const [counts] = await pool.execute(
    `SELECT start_time, COUNT(*) AS total FROM bookings WHERE store_id = ? AND booking_date = ? AND status NOT IN ('cancelled', 'completed') GROUP BY start_time`,
    [storeId, bookingDate],
  );
  const countMap = new Map(counts.map((row) => [String(row.start_time).slice(0, 5), Number(row.total || 0)]));

  const slots = DEFAULT_TIMES.map((time) => {
    const start = new Date(`${bookingDate}T${time}:00`);
    if (start.getTime() <= Date.now()) return { time, available: false, reason: '已超過目前時間' };
    if (blockMap.has(time)) return { time, available: false, reason: blockMap.get(time) };
    if ((countMap.get(time) || 0) >= SLOT_CAPACITY) return { time, available: false, reason: '此時段已額滿' };
...
export async function listBookings(req, res) {
  ...
  res.json({ bookings: rows.map(addCanModify(req)) });
}
...
export async function createBooking(req, res) {
  const { pet_id, service_id, service_ids, store_id, booking_date, start_time, note } = req.body;
  const normalizedServiceIds = normalizeServiceIds(service_ids, service_id);
  if (!pet_id || normalizedServiceIds.length === 0 || !store_id || !booking_date || !start_time) {
    return res.status(400).json({ message: '請填寫完整預約資料' });
  }
  const start = startDateTime(booking_date, start_time);
  if (Number.isNaN(start.getTime())) return res.status(400).json({ message: '預約日期或時間格式錯誤' });
  if (start.getTime() <= Date.now()) return res.status(400).json({ message: '不可預約過去的時間' });

  const pet = await loadPetForUser(pet_id, req.user.id);
  if (!pet) return res.status(403).json({ message: '這不是你的寵物資料' });

  const services = await loadActiveServices(normalizedServiceIds);
  if (services.length !== normalizedServiceIds.length) return res.status(400).json({ message: '部分服務項目不存在或已停用' });

  const invalid = services.find((service) => !serviceTargetAllowed(service, pet.type));
  if (invalid) return res.status(400).json({ message: `${invalid.name} 不適用於${petTypeLabel(pet.type)}` });

  const unavailableSlot = await isSlotUnavailable({ store_id, booking_date, start_time });
  if (unavailableSlot.unavailable) return res.status(400).json({ message: unavailableSlot.reason });

  const unavailableCapacity = await checkCapacity({ storeId: store_id, bookingDate: booking_date, petType: pet.type, services });
  if (unavailableCapacity.unavailable) return res.status(400).json({ message: unavailableCapacity.reason });

  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();
    const serviceOrder = new Map(normalizedServiceIds.map((id, index) => [id, index]));
    services.sort((a, b) => serviceOrder.get(a.id) - serviceOrder.get(b.id));
    const primaryServiceId = services[0].id;
    const [result] = await connection.execute(
      `INSERT INTO bookings (customer_id, pet_id, service_id, store_id, booking_date, start_time, note, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')`,
      [req.user.id, pet_id, primaryServiceId, store_id, normalizeDate(booking_date), normalizeTime(start_time), note || null],
    );
    await insertBookingServices(connection, result.insertId, services);
    await connection.commit();
    res.status(201).json({ id: result.insertId, message: '預約已建立' });
  } catch (error) {
    await connection.rollback();
    console.error('Create booking error:', error);
    res.status(500).json({ message: '建立預約失敗，請稍後再試' });
  } finally {
    connection.release();
  }
}
...
export async function updateBooking(req, res) {
  ...
  if (!ensureBookingOwnerOrStaff(req, booking)) return res.status(403).json({ message: '無權限修改此預約' });
  if (['completed', 'cancelled'].includes(booking.status)) return res.status(400).json({ message: '此預約狀態不可修改' });
  if (!roleCanModify(req, booking)) {
    return res.status(400).json({ message: req.user.role === 'customer' ? '服務開始前 4 小時內不可修改預約' : '服務開始後不可修改預約' });
  }

  const nextPetId = pet_id || booking.pet_id;
  const nextStoreId = store_id || booking.store_id;
  const nextDate = booking_date || normalizeDate(booking.booking_date);
  const nextTime = normalizeTime(start_time || booking.start_time);
  const nextServiceIds = normalizeServiceIds(service_ids, service_id);

  const pet = req.user.role === 'customer' ? await loadPetForUser(nextPetId, req.user.id) : await loadPetById(nextPetId);
  if (!pet) return res.status(req.user.role === 'customer' ? 403 : 404).json({ message: req.user.role === 'customer' ? '這不是你的寵物資料' : '找不到寵物資料' });

  let services = [];
  if (nextServiceIds.length > 0) {
    services = await loadActiveServices(nextServiceIds);
    if (services.length !== nextServiceIds.length) return res.status(400).json({ message: '部分服務項目不存在或已停用' });
    const invalid = services.find((service) => !serviceTargetAllowed(service, pet.type));
    if (invalid) return res.status(400).json({ message: `${invalid.name} 不適用於${petTypeLabel(pet.type)}` });
  }

  const unavailableSlot = await isSlotUnavailable({ store_id: nextStoreId, booking_date: nextDate, start_time: nextTime, excludeBookingId: id });
  if (unavailableSlot.unavailable) return res.status(400).json({ message: unavailableSlot.reason });

  if (services.length > 0) {
    const unavailableCapacity = await checkCapacity({ storeId: nextStoreId, bookingDate: nextDate, petType: pet.type, services, excludeBookingId: id });
    if (unavailableCapacity.unavailable) return res.status(400).json({ message: unavailableCapacity.reason });
  }
  ...
  try {
    ...
    if (nextServiceIds.length > 0) {
      const serviceOrder = new Map(nextServiceIds.map((serviceId, index) => [serviceId, index]));
      services.sort((a, b) => serviceOrder.get(a.id) - serviceOrder.get(b.id));
      nextPrimaryServiceId = services[0].id;
      await connection.execute('DELETE FROM booking_services WHERE booking_id = ?', [id]);
      await insertBookingServices(connection, id, services);
    }
    await connection.execute(
      `UPDATE bookings SET pet_id = ?, service_id = ?, store_id = ?, booking_date = ?, start_time = ?, note = ? WHERE id = ?`,
      [nextPetId, nextPrimaryServiceId, nextStoreId, nextDate, nextTime, note ?? booking.note, id],
    );
    await connection.commit();
    ...
    res.status(500).json({ message: '更新預約失敗，請稍後再試' });
  } finally {
    connection.release();
  }
}
